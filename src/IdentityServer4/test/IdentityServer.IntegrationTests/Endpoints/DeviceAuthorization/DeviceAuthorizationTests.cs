...
public class DeviceAuthorizationTests
{
    ...
    public async Task get_should_return_InvalidRequest()
    {
        ...
        var resultDto = await ParseJsonBody<ErrorResultDto>(response);
    ...
    public async Task valid_should_return_json()
    {
        ...
        var resultDto = await ParseJsonBody<ResultDto>(response);
    ...
    private async Task<T> ParseJsonBody<T>(HttpResponseMessage response)
    {
        var streamBody = await response.Content.ReadAsStreamAsync();
        return await JsonSerializer.DeserializeAsync<T>(streamBody);
    }
