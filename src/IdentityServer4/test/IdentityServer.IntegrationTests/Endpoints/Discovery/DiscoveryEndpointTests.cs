...
public class DiscoveryEndpointTests
{
    ...
    public async Task Issuer_uri_should_be_lowercase()
    {
        ...
        var issuer = data.RootElement.GetProperty("issuer").GetString();
    ...
    public async Task Algorithms_supported_should_match_signing_key()
    {
        ...
        var json = await result.Content.ReadAsStringAsync();
        var data = JsonDocument.Parse(json);
        var algorithmsSupported = data.RootElement.GetProperty("id_token_signing_alg_values_supported").EnumerateArray();

        algorithmsSupported.Count().Should().Be(2);

        algorithmsSupported.Select(x => x.GetString()).Should().Contain(SecurityAlgorithms.RsaSha256);
        algorithmsSupported.Select(x => x.GetString()).Should().Contain(SecurityAlgorithms.EcdsaSha256);
    }
    ...
    public async Task Jwks_entries_should_countain_crv()
    {
        ...
        var keys = data.RootElement.GetProperty("keys").EnumerateArray();
        keys.Should().NotBeNull();

        var key = keys.ElementAtOrDefault(1);
    ...
    public async Task Jwks_entries_should_contain_alg()
    {
        ...
        var key = keys.ElementAtOrDefault(0);
        key.Should().NotBeNull();
